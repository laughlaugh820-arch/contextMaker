...
| `--persona` | **書き手の矜持と理念を反映させる**。`context/persona/` の名前か Markdown のパス。指定しなければ作法だけで書かせる。下の「書き手の矜持と理念」 |
| `--persona-stage` | ペルソナをどの段階から渡すか。`write`（既定）は舞台と人物を先にペルソナなしで決めて本文だけに渡す。`all` は最初から渡す |
| `--world-stage` | ペルソナが無くても、舞台と人物の設計表を先に作ってから本文を書く。設計表の効果をペルソナと切り分けるため |
...
### 書き手の矜持と理念（ペルソナ）を反映させるか

生成の指示はコーパスの数値と作法だけでできていて、**書き手が何を譲らず、何を信じて書いているか**は
どこにもない。`context/persona/` に書き手の考え方を置き（矜持、理念、人間観、美しいと思うもの、
恥と思うこと、書く理由、書かないこと、人物への態度）、`--persona <名前>` で渡すと
「この考え方で書く」としてプロンプトの依頼の直後に入る。経歴や職業は書かず、登場人物の設定でもない。
書き手は本文に出さず、矜持と理念は何を描き何を省くか、誰に寄るか、どこで終えるかという選択に出させる。

```console
$ python3 tools/story.py generate "冬の停留所で待つ人" --persona 見たものしか書かない --backend cli
```

ペルソナは既定では本文を書く段階だけに渡す（`--persona-stage write`）。舞台と人物はペルソナなしで先に決め、
理念が舞台に翻訳されるのを防ぐ（最初から渡した『鉄紺』は、伝統美の項目から金沢の茶屋が選ばれた）。
最初から渡すなら `--persona-stage all`。

反映する／しないは生成ごとに選ぶもので、どちらが良いかは決めていない。
同じテーマ・同じ `--seed` であり／なしを1本ずつ作り、`judge.py pair` で比べる。
`meta.json` の `persona` に名前が残り、渡した資料は `persona.md` として生成物の横に写される。
書き方の約束と雛形は `context/persona/README.md`。
