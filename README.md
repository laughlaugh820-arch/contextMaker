...
| `--persona` | **書き手の人間性を反映させる**。`context/persona/` の名前か Markdown のパス。指定しなければ作法だけで書かせる。下の「書き手の人間性」 |
...
### 書き手の人間性（ペルソナ）を反映させるか

生成の指示はコーパスの数値と作法だけでできていて、**誰が書くか**はどこにもない。
`context/persona/` に書き手を一人の人間として置き（生きてきた仕事、人をどう見るか、何度も戻る題材、
書かないこと、人物との距離、笑い、体と生活）、`--persona <名前>` で渡すと「この人間が書く」として
プロンプトの依頼の直後に入る。登場人物の設定ではなく、書き手は本文に出てこない。
人間性は何を描き何を省くか、誰に寄るか、どこで終えるかという選択に出させる。
